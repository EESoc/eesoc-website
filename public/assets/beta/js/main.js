;(function () {
	...
	var clickMenu = function() {
		...
		$('#navbar a:not([class="external"]), #fh5co-home .read-on-btn').click(function(event){
	...
	var sponsorPopup = function() {

		$('[data-sponsor-show]').click(function () {;
			var $this = $(this),
				$qrySel = '[data-sponsor-show=' + $this.data('sponsor-show') + ']';
				//alert($qrySel);
			$('#modal-info-template .modal-title').text($($qrySel + ' .sponsor-title').text().replace(/"/g, ""));
			$('#modal-info-template .modal-body').html($($qrySel + ' .sponsor-excerpt-long').html());

			$('#modal-info-template').css("overflow", "hidden");
			$('#modal-info-template').modal();
			return false;
		});
	...
	// Reflect scrolling in navigation
	var committeePopup = function() {

		$('.more-desc').click(function () {
			var $this = $(this),
				$qrySel = '.team-box[data-committee-show=' + $this.data('committee-show') + ']';
				//alert(sponsorId);
			//alert($($qrySel).text());
			$('#modal-info-template .modal-title').text($($qrySel + ' .committee-title').text() + ' - ' + $($qrySel + ' .committee-role').text());
			$('#modal-info-template .modal-body').html($($qrySel + ' .committee-excerpt-long').html());
	...
	var test = function(){
		...
		fetch("https://allorigins.me/get?url=https://www.facebook.com/events/151230655829890")
            .then( res => res.text() )
            .then( res => {
				...
				let nodes = parser.childNodes[0].childNodes[1].getElementsByClassName("\\\"hidden_elem\\\"");
				console.log(nodes)
